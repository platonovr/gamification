 ...
 */
@Component
public class FirstStudyTaskChecker extends AbstractBadgeChecker {
    @Override
    public boolean isBadgeApplicable(Account account) {
        ...
        return tasksByUser
                ...
                .anyMatch(it -> TaskCategoryType.STUDY.equals(it.getCategory().getTaskCategoryType()));
    }

    @Override
    public Badge getBadge() {
        try {
            return simpleDao.findById(Badge.class, BadgeConstants.FIRST_STUDY_BADGE);
        } catch (Exception e) {
            return null;
        }
    }
